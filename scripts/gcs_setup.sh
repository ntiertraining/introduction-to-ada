...
echo "Installing the latest Alire package..." | tee -a ${pwd}/scripts/setup.log
cd ~/Downloads
alr_latest=$(curl -sL https://api.github.com/repos/alire-project/alire/releases/latest | jq -r .tag_name)
echo "Alire version ${alr_latest}" | tee -a ${pwd}/scripts/setup.log
if ! curl -sLO "https://github.com/alire-project/alire/releases/download/${alr_latest}/alr-${alr_latest#v}-bin-x86_64-linux.zip" >> ${pwd}/scripts/setup.log 2>&1; then
    echo "Failed to download Alire package." | tee -a ${pwd}/scripts/setup.log
    exit 1
fi
sudo unzip -o -j alr-${alr_latest#v}-bin-x86_64-linux.zip bin/alr -d /usr/local/bin >> ${pwd}/scripts/setup.log 2>&1

# Install system dependencies required for building and unpacking Ada tooling

echo "Installing the build dependencies..." | tee -a ${pwd}/scripts/setup.log
sudo apt-get update >> ${pwd}/scripts/setup.log 2>&1
if ! sudo apt-get install -y --no-install-recommends build-essential gnat gprbuild >> ${pwd}/scripts/setup.log 2>&1; then
    echo "Failed to install build dependencies." | tee -a ${pwd}/scripts/setup.log
    exit 1
fi
...
if ! /google/devshell/editor/code-oss-for-cloud-shell/bin/remote-cli/codeoss --install-extension adacore.ada >> ${pwd}/scripts/setup.log 2>&1; then
    echo "Failed to install Visual Studio Code extension." | tee -a ${pwd}/scripts/setup.log
    exit 1
fi
