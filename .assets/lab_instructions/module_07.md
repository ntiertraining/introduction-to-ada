<h1><img alt="nTier Logo" style="margin-bottom: -10px;" src="../images/ntier-logo.png">&nbsp;&nbsp; Module 07: Packages and Modular Programming</h1>
...
3. In `src/types/`, create `telemetry_types.ads`. Move the following into
   this file, inside `package Telemetry_Types is ... end Telemetry_Types;`:
   - `Min_Airspeed`, `Max_Airspeed`, `Min_Altitude`, `Max_Altitude`,
     `Min_G_Force`, and `Max_G_Force`
   - `Critical_Angle_Of_Attack`
   - `Airspeed_Type`, `Altitude_Type`, and `G_Force_Type`
   - `Wing_Area` and `Angle_Of_Attack`
   - `Flight_Mode_Type`
   - `Air_Data_Type`

   This package holds only type and constant declarations, no subprogram,
   so it needs no matching body file; a package with a specification and
   no body is legal whenever the specification declares nothing that
...
7. In the same directory, create `telemetry_generation.adb`. Move the
   following here:
   - `Airspeed_Increment`, `G_Force_Increment`, and `Angle_Increment`, as
     private constants
   - the bodies of `Initialize_Telemetry` and `Apply_Increments`

   A completing body's default parameter expressions must match the
   specification's `:=` defaults exactly, or be absent from both; repeat
   the parameter names, types, and the identical `:=` defaults from the
   specification in the body.
8. In `src/reporting/`, create `flight_reporting.ads`, withing and using
   `Telemetry_Types`. Declare the following, matching their Module 6
   profiles:
   - `Report_Attitude`
   - `Report_Posture`
   - `Report_Event` (two-parameter overload)
   - `Report_Event` (three-parameter overload)
...
10. In `flight_reporting.ads` and `flight_reporting.adb`, declare and
    implement `Report_Configuration`, taking `Aircraft_Weight`,
    `Flight_Mode`, `Weapons_Armed`, and `Data : Air_Data_Type` as
    parameters. Move the startup `Put_Line` block that currently sits in
    `flight_deck.adb` right after `begin` — everything from
    `"Cockpit Telemetry Data Set"` through the `Wing_Loading` and
    `Airspeed_Fraction` calculations and their two reporting
    `Put_Line`s, including the separator line that closes out that
    block — into this new procedure's body. `Wing_Loading` and
    `Airspeed_Fraction` become local variables declared inside
    `Report_Configuration` itself; nothing outside that block ever reads
    either one. Call `Report_Configuration` from `flight_deck.adb` in
    place of the block just moved.
11. In `src/logging/`, create `event_logging.ads`, withing and using
    `Telemetry_Types`. Declare `Event_Log_Type` and `Event_Log_Access` as
    in Module 6, plus the following:
    - `function Build_Event_Log (Data : Air_Data_Type;
...
12. In the same directory, create `event_logging.adb`, withing
    `Ada.Text_IO` and `Ada.Unchecked_Deallocation`.
    - Move the `Ada.Unchecked_Deallocation` instantiation here.
    - Move the body of `Print_Log_Entry`, unchanged from Module 6.
    - Write `Build_Event_Log`'s body as in Module 6.
    - Write `Free_Log_Entry`'s body to check `Item /= null` and call the
      instantiated `Free` procedure only when the check passes; Module 6
      placed that same null check at every call site inside
      `flight_deck.adb`, and moving it into `Free_Log_Entry` means every
      caller gets the check automatically, with one place to maintain it.
13. Rewrite `src/flight_deck.adb`. Replace the type, subtype, and
    constant declarations moved out in steps 3 through 12 with `with`
    clauses for the following packages:
    - `Telemetry_Types`, paired with a `use` clause
    - `Telemetry_Generation`, paired with a `use` clause
    - `Flight_Reporting`, paired with a `use` clause
    - `Event_Logging`, paired with a `use` clause
    - `Flight_Physics`, without a paired `use` clause; every call to
      `Safety_Ratio` or `Is_Stall_Risk` inside `flight_deck.adb` from this
      point forward reads `Flight_Physics.Safety_Ratio` and
      `Flight_Physics.Is_Stall_Risk`, the package name written out at the
      call site.

    The four packages given a `use` clause supply names not easily
    confused with anything else in this file; the two physics functions
    sit closer to the center of what this file actually computes, and
    spelling out their package keeps that computation visually distinct
    from reporting and logging calls around it.
14. Keep `Event_Count`, `Continue_Simulation`, `Aircraft_Weight`,
    `Flight_Mode`, and `Weapons_Armed` declared directly in
    `flight_deck.adb`; none of the five packages needs them declared
    inside itself, even though `Aircraft_Weight`, `Flight_Mode`, and
    `Weapons_Armed` now also get passed into `Report_Configuration` as
    arguments. `Wing_Loading` and `Airspeed_Fraction` no longer belong in
    `flight_deck.adb` at all; step 10 moved both into
    `Report_Configuration` as its own local variables.
15. Keep the `Event_Log_Array` type declaration and the `Event_Log`
...
16. Keep `Report_Log` and `Run_Flight_Events` declared directly in
...
17. Compile the project. Confirm the build now compiles six separate
...
18. Edit only `flight_physics.adb`, changing `Base_Stall_Speed` to a
    ...
    "Doesn't get recompiled" and "doesn't matter" are not the same
    claim, though: the linker still always links the freshest
    `flight_physics.o` into the final executable, so the running
    program's behavior changes everywhere `Base_Stall_Speed` matters —
    every call chain that reaches `Flight_Physics` — regardless of which
    of the five compilation units GNAT chose to retranslate. Which
    source files got recompiled and whether the change takes effect are
    easy to conflate but are not the same question. Restore
    `Base_Stall_Speed` afterward.
